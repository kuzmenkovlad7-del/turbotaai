export const ru = {
  // Navigation
  Home: "Главная",
  About: "О нас",
  Services: "Услуги",
  Pricing: "Цены",
  Contact: "Контакты",
  "Sign In": "Войти",
  "Sign Up": "Регистрация",

  // Hero Section
  "AI-Powered Psychological Support": "Психологическая поддержка на базе ИИ",
  "Professional, personalized psychological care through advanced AI technology. Connect through voice, chat, or video calls with our AI psychologist.":
    "Профессиональная, персонализированная психологическая помощь с использованием передовых технологий ИИ. Свяжитесь через голосовые звонки, чат или видеозвонки с нашим ИИ-психологом.",
  "Get Started": "Начать",
  "Learn More": "Узнать больше",
  "First 5 minutes free, then pay-as-you-go for continued support.":
    "Первые 5 минут бесплатно, затем оплата по факту для продолжения поддержки.",

  // Connection Options
  "How would you like to connect?": "Как вы хотите связаться?",
  "Call an AI Psychologist": "Позвонить ИИ-психологу",
  "Speak directly with our AI psychologist for immediate support.":
    "Говорите напрямую с нашим ИИ-психологом для немедленной поддержки.",
  "Start Voice Call": "Начать голосовой звонок",
  "Chat with AI": "Чат с ИИ",
  "Text-based conversation with our AI for support at your own pace.":
    "Текстовый разговор с нашим ИИ для поддержки в вашем темпе.",
  "Start Chat": "Начать чат",
  "Video Call with AI": "Видеозвонок с ИИ",
  "Face-to-face session with our 3D animated AI psychologist.":
    "Сессия лицом к лицу с нашим 3D анимированным ИИ-психологом.",
  "Start Video Call": "Начать видеозвонок",

  // Features
  "Key Features": "Ключевые особенности",
  "Our AI-powered psychological support service combines cutting-edge technology with professional care principles.":
    "Наш сервис психологической поддержки на базе ИИ сочетает передовые технологии с принципами профессиональной заботы.",
  "First 5 Minutes Free": "Первые 5 минут бесплатно",
  "Try our service with no commitment. First 5 minutes of any session are completely free.":
    "Попробуйте наш сервис без обязательств. Первые 5 минут любой сессии полностью бесплатны.",
  "Pay As You Go": "Оплата по факту",
  "Only pay for the time you need. No subscriptions or hidden fees.":
    "Платите только за то время, которое вам нужно. Без подписок и скрытых платежей.",
  "Personalized Experience": "Персонализированный опыт",
  "Our AI remembers your past sessions to provide better support over time.":
    "Наш ИИ запоминает ваши прошлые сессии, чтобы обеспечить лучшую поддержку со временем.",
  "Multilingual Support": "Многоязычная поддержка",
  "Automatic language detection and support across all communication channels.":
    "Автоматическое определение языка и поддержка на всех каналах связи.",
  "Emotion Recognition": "Распознавание эмоций",
  "Advanced analysis of text and facial expressions to better understand your needs.":
    "Продвинутый анализ текста и выражения лица для лучшего понимания ваших потребностей.",
  "Private & Secure": "Конфиденциально и безопасно",
  "Your data and conversations are protected with enterprise-grade security.":
    "Ваши данные и разговоры защищены системой безопасности корпоративного уровня.",

  // Team Section
  "Our Team": "Наша команда",
  "Our team of AI psychology specialists": "Наша команда специалистов по ИИ-психологии",
  "Passionate About Mental Health": "Увлечены психическим здоровьем",
  "Combining expertise in psychology and AI": "Объединяем опыт в психологии и ИИ",
  "Our diverse team brings together expertise in psychology, artificial intelligence, and user experience design. We're committed to making mental health support accessible to everyone through innovative technology.":
    "Наша разнообразная команда объединяет опыт в психологии, искусственном интеллекте и дизайне пользовательского опыта. Мы стремимся сделать поддержку психического здоровья доступной для всех с помощью инновационных технологий.",

  // Footer
  "AI Psychological Support": "Психологическая поддержка ИИ",
  "Professional, scalable, and aesthetically pleasing online service that utilizes AI to deliver quality psychological care.":
    "Профессиональный, масштабируемый и эстетически приятный онлайн-сервис, использующий ИИ для предоставления качественной психологической помощи.",
  "Quick Links": "Быстрые ссылки",
  "About Us": "О нас",
  Legal: "Юридическая информация",
  "Terms of Service": "Условия использования",
  "Privacy Policy": "Политика конфиденциальности",
  "Cookie Policy": "Политика использования файлов cookie",
  "GDPR Compliance": "Соответствие GDPR",
  "Automatic language detection": "Автоматическое определение языка",

  // Misc
  "AI Psychologist": "ИИ-психолог",
  "Select Language": "Выбрать язык",
  "Search languages": "Поиск языков",
  "All rights reserved": "Все права защищены",
  "AI psychology concept with heart and brain icons": "Концепция ИИ-психологии с иконками сердца и мозга",
  "Learn about our AI psychology service": "Узнайте о нашем сервисе ИИ-психологии",
  "About our AI psychology service": "О нашем сервисе ИИ-психологии",
  "Detected language": "Обнаруженный язык",
  Language: "Язык",

  // Auth Unavailable Messages
  "Authentication is temporarily unavailable. Please contact support.":
    "Авторизация временно недоступна. Пожалуйста, свяжитесь с поддержкой.",
  "Авторизация временно отключена": "Авторизация временно отключена",
  "Регистрация временно недоступна": "Регистрация временно недоступна",
  "Это демо-окружение. Для работы с реальной авторизацией добавьте ключи Supabase в файл .env.local:":
    "Это демо-окружение. Для работы с реальной авторизацией добавьте ключи Supabase в файл .env.local:",
  "Вернуться на главную": "Вернуться на главную",
  Guest: "Гость",

  // Assistant Modal Translations
  "AI Psychologist (Chat)": "ИИ-психолог (Чат)",
  "AI Psychologist (Voice Call)": "ИИ-психолог (Голосовой звонок)",
  "AI Psychologist (Video Call)": "ИИ-психолог (Видеозвонок)",
  "Choose Your AI Psychologist": "Выберите AI-психолога",
  "Select the AI psychologist you'd like to speak with during your video call.":
    "Выберите ИИ-психолога, с которым вы хотите поговорить во время видеозвонка.",
  "Voice communication language": "Язык голосового общения",
  "Video call language": "Язык видеозвонка",
  "Chat language": "Язык чата",
  "AI will understand and respond in this language with native accent":
    "ИИ будет понимать и отвечать на этом языке с родным акцентом",
  "Choose AI voice gender:": "Выберите пол голоса ИИ:",
  "Start Session": "Начать сессию",
  "Start Call": "Начать звонок",
  "End Call": "Завершить звонок",
  "Close": "Закрыть",
  User: "Пользователь",

  // Services Page
  "Our Services": "Наши услуги",
  "Explore our range of AI-powered psychological support services designed to meet your unique needs.":
    "Изучите наш спектр услуг психологической поддержки на базе ИИ, разработанных для удовлетворения ваших уникальных потребностей.",
  "Voice Support": "Голосовая поддержка",
  "Our voice AI system provides real-time conversation with natural language understanding. Speak naturally and receive thoughtful responses and guidance. Perfect for those who prefer verbal communication and want a more personal experience.":
    "Наша голосовая система ИИ обеспечивает разговор в реальном времени с пониманием естественного языка. Говорите естественно и получайте продуманные ответы и руководство. Идеально подходит для тех, кто предпочитает устное общение и хочет более личного опыта.",
  "Immediate Response": "Мгновенный ответ",
  "Get instant support without waiting for an appointment.":
    "Получите мгновенную поддержку без ожидания записи на прием.",
  "Secure Conversations": "Безопасные разговоры",
  "All voice data is encrypted and processed securely.": "Все голосовые данные зашифрованы и обрабатываются безопасно.",
  "Voice Analysis": "Анализ голоса",
  "Our AI can detect emotional cues in your voice to provide better support.":
    "Наш ИИ может обнаруживать эмоциональные сигналы в вашем голосе для обеспечения лучшей поддержки.",
  "Self-Paced": "В своем темпе",
  "Take your time to express yourself without pressure.": "Не торопитесь выражать свои мысли без давления.",
  "Conversation History": "История разговоров",
  "Access your chat history to review previous sessions.":
    "Получите доступ к истории чата для просмотра предыдущих сессий.",
  "Text Analysis": "Анализ текста",
  "Advanced natural language processing to understand context and emotions.":
    "Продвинутая обработка естественного языка для понимания контекста и эмоций.",
  "Visual Interaction": "Визуальное взаимодействие",
  "See facial expressions and body language for a more personal experience.":
    "Видите выражения лица и язык тела для более личного опыта.",
  "Secure Video": "Безопасное видео",
  "End-to-end encrypted video sessions for complete privacy.":
    "Сквозное шифрование видеосессий для полной конфиденциальности.",
  "Compare Our Services": "Сравните наши услуги",
  Feature: "Функция",
  "Real-time Interaction": "Взаимодействие в реальном времени",
  "Visual Cues": "Визуальные сигналы",
  "Voice Tone Analysis": "Анализ тона голоса",
  "Self-paced Communication": "Общение в своем темпе",
  "Ready to get started?": "Готовы начать?",
  "Choose the service that best fits your needs and preferences. All services include our first 5 minutes free guarantee.":
    "Выберите услугу, которая лучше всего соответствует вашим потребностям и предпочтениям. Все услуги включают гарантию первых 5 минут бесплатно.",

  // Hero Section - New Keys
  "Myitra Platform · AI + Psychology": "Платформа Myitra · ИИ + Психология",
  "Live Psychological Support,": "Живая психологическая поддержка,",
  "AI-Enhanced": "усиленная ИИ",
  "Licensed psychologists supported by AI assistants. We help gather history, maintain journals, and remind about sessions.":
    "Лицензированные психологи с поддержкой ИИ-ассистентов. Помогаем собрать анамнез, вести дневник и напоминаем о сессиях.",
  "Talk Now": "Поговорить сейчас",
  "View Services": "Посмотреть услуги",
  "AI Chat 24/7": "ИИ-чат 24/7",
  "Voice Calls": "Голосовые звонки",
  "Video Sessions": "Видеосессии",
  "Myitra Psychology Session": "Психологическая сессия Myitra",
  "3 Assistant Modes": "3 режима ассистента",
  "chat · voice · video": "чат · голос · видео",
  "Welcome Back": "Добро пожаловать снова",
  "Sign in to continue your journey with AI Psychology": "Войдите, чтобы продолжить свой путь с ИИ-психологией",
  "Enter your credentials to access your account": "Введите свои данные для доступа к аккаунту",
  "Signing in...": "Вход...",
  "Forgot password?": "Забыли пароль?",
  "Hide password": "Скрыть пароль",
  "Show password": "Показать пароль",
  "Create account": "Создать аккаунт",
  "By signing in, you agree to our": "Входя, вы соглашаетесь с нашими",
  "and": "и",
}
