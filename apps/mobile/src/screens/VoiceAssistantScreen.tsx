 ...
 *
 * Opens the web voice-call dialog inside an authenticated WebView.
 * The WebView receives the auth token and device hash via injected JS
 * so the user session is preserved — no re-login needed.
 *
 * This approach provides full web feature parity (speech recognition,
 * TTS with accent support, waveform visualization) while keeping
 * the native navigation and preserving existing voice env logic.
 */
export default function VoiceAssistantScreen() {
  ...
  const uri = `${API_BASE_URL}/?mode=voice&lang=${locale}&mobile=1`
