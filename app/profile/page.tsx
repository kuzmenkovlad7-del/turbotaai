      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="text-right">{loading ? "…" : summary?.paid_until ? new Date(summary.paid_until).toLocaleString() : "Не активно"}</div>

            <div className="text-gray-500">Промо до:</div>
            <div className="text-right">{loading ? "…" : summary?.promo_until ? new Date(summary.promo_until).toLocaleString() : "Не активно"}</div>
          </div>

          {/* ✅ ВАЖНО: увеличиваем расстояние между кнопками */}
          <div className="mt-5 grid gap-3">
            ...
            <Link
              href="/pricing"
              className="w-full rounded-xl border px-4 py-2 text-center text-sm"
            >
              Тарифы
            </Link>
          </div>
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 text-sm text-gray-600">
            {summary?.logged_in
              ? "Здесь будут кнопки отмены автопродления и промо."
              : "Войдите, чтобы управлять подпиской и промо."}
          </div>
          ...
          {/* ✅ Убрали debug 'status: inactive' полностью */}
        </div>
      </div>
