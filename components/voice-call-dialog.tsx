...
const MIN_BLOB_BYTES = 4500
const RESTART_COOLDOWN_MS = 120
const MIN_THR = 0.003
...
export default function VoiceCallDialog({
  ...
  // refs (чтобы в async не ловить stale state)
  ...
  function cleanupAudioGraph(opts?: { close?: boolean }) {
    stopMonitoring()
    ...
    ;(async () => {
      try {
        ...
        if (!res.ok || !data || data.success === false || !data.audioContent) {
          ...
          if (isCallActiveRef.current && !isMicMutedRef.current) setIsListening(true)
          return
        }
  ...
  async function startUtteranceRecorder(stream: MediaStream) {
    ...
    rec.onstop = async () => {
      ...
      const dur = Date.now() - utteranceStartAtRef.current
      const type = rec.mimeType || chunksRef.current[0]?.type || "application/octet-stream"
      const blob = new Blob(chunksRef.current, { type })
      chunksRef.current = []

      dlog("[REC] stop -> blob", { size: blob.size, type, dur })
  ...
  function startVadLoop() {
    ...
    const tick = () => {
      ...
      if (!recordingRef.current && isVoice && voiceActiveMsRef.current >= VOICE_START_MS) {
        setIsListening(true)
        void startUtteranceRecorder(stream)
      }
      ...
      if (debugEnabled && now % 500 < 17) {
        dlog("[VAD]", { rms: +rms.toFixed(4), thr: +thr.toFixed(4), voice: isVoice, rec: recordingRef.current, ctx: ctx?.state })
      }
  ...
  function buildAudioGraphWithExistingContext(ctx: AudioContext, stream: MediaStream) {
    try {
      ...
      console.error("[AUDIO] build graph failed", e)
    }
  }
  ...
  async function recoverMic() {
    ...
    try {
      ...
      resetCalibration()
      buildAudioGraphWithExistingContext(ctx, stream)
      ...
      setIsListening(true)
      dlog("[MIC] recovered", { ctx: ctx.state })
    } catch (e: any) {
      console.error("[MIC] recover failed", e)
  ...
  function attachTrackHandlers(stream: MediaStream) {
    ...
    tr.onended = () => {
      ...
      if (isCallActiveRef.current) void recoverMic()
    }
    tr.onmute = () => dlog("[MIC] muted")
    tr.onunmute = () => dlog("[MIC] unmuted")
  }
  ...
  const startCall = async (gender: Gender) => {
    ...
    try {
      ...
      // КЛЮЧЕВОЕ: создаём AudioContext ДО первого await (это фиксит ПК/Chrome)
      cleanupAudioGraph({ close: true })
      let ctx = audioCtxRef.current
      if (!ctx) {
        const AudioCtx = (window.AudioContext || (window as any).webkitAudioContext) as typeof AudioContext
        ctx = new AudioCtx()
        audioCtxRef.current = ctx
        dlog("[AUDIO] ctx created", { state: ctx.state, ua: (navigator as any).userActivation?.isActive })
      }
      if (ctx.state === "suspended") {
        void ctx.resume()
          .then(() => dlog("[AUDIO] ctx resumed", { state: ctx?.state }))
          .catch((e) => dlog("[AUDIO] resume failed", e))
      }
